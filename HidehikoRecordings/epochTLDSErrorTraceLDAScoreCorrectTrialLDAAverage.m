...
for nSession =3 % 1:numSession
    ...
    subplot(1, 3, 1)
    hold on
    shadedErrorBar(params.timeSeries, abs(mean(simCorrMat)),std(simCorrMat)/sqrt(length(simCorrMat)),{'-k','linewid',1},0.5);
    ...
    setPrint(8*3, 6*1, ['Plots/TLDSLDASimilarityErrorSesssionCorrectLDAAve_idx_' num2str(nSession, '%02d') '_xDim_' num2str(xDim)])
