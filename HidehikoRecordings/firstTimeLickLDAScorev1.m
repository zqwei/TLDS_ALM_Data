...
%         scoreMat(:, nTime) = scoreMat(:, nTime)./var(scoreMat(:, nTime));
    ...
    figure;
    hold on;
    [fi, xi] = histcounts(mean(scoreMat(totTargets, timePoint(end)+slideWin), 2), -4:0.1:4);
    ...
    figure;
    hold on
%     plot(mean(scoreMat(totTargets & firstLickTime > fltThres, timePoint(end)+slideWin), 2), firstLickTime(totTargets & firstLickTime > fltThres), 'ob')
    plot(mean(scoreMat(totTargets, timePoint(end)+slideWin), 2), firstLickTime(totTargets), 'ob')
    [p, h] = corr(mean(scoreMat(totTargets & firstLickTime > fltThres, timePoint(end)+slideWin), 2), firstLickTime(totTargets & firstLickTime > fltThres), 'type', 'Spearman');
