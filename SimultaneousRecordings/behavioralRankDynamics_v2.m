...
for nData = 1:length(nDataSet)
    ...
    early_trial   = nBehaviors(nData).behavior_early_report(max(trial_index-1, 1));
    reward        = nBehaviors(nData).behavior_early_report==0 & nBehaviors(nData).behavior_report==1;
    reward_trial  = reward(max(trial_index-1, 1));
    ...
    stim_trial    = stim(max(trial_index-1, 1));
    trial_type    = nBehaviors(nData).task_trial_type(max(trial_index-1, 1))==1;
    
    % KF
    nSessionData  = [nDataSet(nData).unit_KF_yes_fit; nDataSet(nData).unit_KF_no_fit];
    nSessionData  = normalizationDim(nSessionData, 2); 
    correctRate   = coeffSVM(nSessionData, reward_trial);    
...
for nPlot = 1:7
    ...
    hold on
    plot(ave_reward(:, 1), ave_reward(:, nPlot+1), 'ob')
    p = signrank(ave_reward(:, 1), ave_reward(:, nPlot+1), 'tail', 'both')
    plot([0 1], [0, 1], '--k')
%     xlim([0 0.6])
%     ylim([0 0.6])
    xlabel('SAS')
