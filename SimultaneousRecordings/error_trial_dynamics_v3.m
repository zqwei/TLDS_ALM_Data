...
for nSession = [13 14 17 25 28 33]
    ...
    hold on
    shadedErrorBar(param.timeSeries, mean(simCorrMat(timePoint(3)-2:timePoint(3)+2, :)), std(simCorrMat(timePoint(3)-2:timePoint(3)+2, :))/sqrt(5), {'-g'})   
    ...
    simCorrMat    = getGaussianPSTH(filterInUse, simCorrMat);
    hold on
    shadedErrorBar(param.timeSeries, mean(simCorrMat(timePoint(3)-2:timePoint(3)+2, :)), std(simCorrMat(timePoint(3)-2:timePoint(3)+2, :))/sqrt(5), {'-m'})   
    ylim([0 1]);
    ...
    subplot(2,1,2) % reproduce of figure 5e
    hold on
    shadedErrorBar(param.timeSeries, mean(simCorrMat(timePoint(3)-2:timePoint(3)+2, :)), std(simCorrMat(timePoint(3)-2:timePoint(3)+2, :))/sqrt(5), {'-g'})   
    ...
    setPrint(8, 6*2, ['Plots/Error_Trial_Rank_Sample_idx_' num2str(nSession)])
    close all
end