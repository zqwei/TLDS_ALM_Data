...
for nData = 1:length(nDataSet)
    ...
    for nEpoch     = 1:length(timePoint)-1
        scoreMat_  = scoreMat(:, timePoint(nEpoch):timePoint(nEpoch+1));
        trialFullCorrEpoch(1, nEpoch) = mean(corr(scoreMat_(1:numYesTrial, :), (1:numYesTrial)', 'type', 'spearman'));
        trialFullCorrEpoch(2, nEpoch) = mean(corr(scoreMat_(1+numYesTrial:end,:), (1:(numTrials-numYesTrial))', 'type', 'spearman'));
    end
