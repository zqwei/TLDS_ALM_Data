...
function errorDataSet  = getSimultaneousErrorSpikeDataDirList(nDataSet, mDataSet, SpikeHiFileList)
    ...
    for nSession       = 1:length(nDataSet)
        ...
        if ~isempty(intSect)
            ...
            for nIntSect = 1:length(intSect)
                ...
                load([SpikeHiFileList(nDataSet(nSession).sessionIndex).folder '/' SpikeHiFileList(nDataSet(nSession).sessionIndex).name])
