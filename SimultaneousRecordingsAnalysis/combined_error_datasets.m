...
for nSession = 1:numSession
    ...
    % GPFA
    load(['../LiSimultaneousRecordings/GPFAFits/gpfa_optxDimErrFit_idx_' num2str(nSession) '.mat'])
%     y_est = nan(size(Y));
...
for nSession = 1:numSessionHi
    ...
    % GPFA
    load(['../HidehikoRecordings/GPFAFits/gpfa_optxDimErrFit_idx_' num2str(nSession) '.mat'])
%     y_est = nan(size(Y));
%     for nTrial = 1:size(Y, 3)
%         y_est_nTrial = estParams.C*seqTrain(nTrial).xsm;
%         y_est(:, :, nTrial) = y_est_nTrial;
...
load([TempDatDir 'SimultaneousError_HiSoundSpikes.mat'])
mean_type    = 'Constant_mean';
tol          = 1e-6;
cyc          = 10000;
...
for nSession = 1:numSessionSound
    if ~isempty(nDataSet(nSession).unit_yes_trial) && ~isempty(nDataSet(nSession).unit_no_trial)
        ...
        if optFit>0
            ...
            % GPFA
            load(['../HidehikoSoundTasks/GPFAFits/gpfa_optxDimErrFit_idx_' num2str(nSession) '.mat'])
    %         y_est = nan(size(Y));
    %         for nTrial = 1:size(Y, 3)
    %             y_est_nTrial = estParams.C*seqTrain(nTrial).xsm;
    ...
    %         end
    %         y_est = permute(y_est, [3 1 2]);
            nDataSet(nSession).unit_GPFA_yes_fit = y_est(1:numYesTrial, :, 1:T);
            nDataSet(nSession).unit_GPFA_no_fit  = y_est(1+numYesTrial:end, :, 1:T);
        end
    end
end
