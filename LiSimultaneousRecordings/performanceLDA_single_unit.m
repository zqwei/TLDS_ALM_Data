...
for nSession = 1:numSession
    ...
    L          = nan(size(neuroMat, 1), kFold);
    
    for nUnit  = 1:size(neuroMat, 1)
        L(nUnit, :) = 1 - kfoldLoss(fitcdiscr(neuroMat(nUnit, :)', totTargets, ...
                     'discrimType','pseudoLinear', 'KFold', kFold),...
                     'lossfun', 'classiferror', 'mode', 'individual');
    end
    [LDA_decode(nSession,1), id_max]= max(mean(L, 2));
    LDA_decode(nSession,2) = std(L(id_max, :))/sqrt(kFold);
...
for nSession = 1:numSessionHi
    ...
    [LDA_decode(nSession+numSession,1), id_max]= max(mean(L, 2));
    LDA_decode(nSession+numSession,2) = std(L(id_max, :))/sqrt(kFold);
...
set(gca, 'TickDir', 'out')
box off
setPrint(8, 6, 'Plots/Performance_TLDSScore_single_unit')
